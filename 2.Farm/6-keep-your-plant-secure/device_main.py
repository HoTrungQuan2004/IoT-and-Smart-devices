...
x509 = X509(
    cert_file="device/certs/plant-guardian-x509-cert.pem",
    key_file="device/certs/plant-guardian-x509-key.pem",
    pass_phrase=None
)
...
device_client = IoTHubDeviceClient.create_from_x509_certificate(
    ...
    device_id="plant-guardian-x509",
    x509=x509
)

# Connect to IoT Hub
device_client.connect()
print("Connected to plant-secure-hub-789")
...
current_moisture = 600  # Starting value (0-1023 range)

# Main loop with dynamic simulation
while True:
    # Simulate soil drying out (decrease moisture randomly)
    current_moisture = max(0, current_moisture - random.randint(5, 15))
    ...
    message = Message(json.dumps({
        ...
        "device": "plant-guardian-x509",
        "status": "running"
    }))
    device_client.send_message(message)
    print(f"Telemetry sent: Soil Moisture = {moisture_reading}")
